...
Common fixes for Cyberfield Analytics issues.

## Password reset

1. Go to app.cyberfield-analytics.example/login
2. Click **Forgot password**
3. Enter the email on your Cyberfield account
4. Open the reset link (valid for **60 minutes**)
5. Choose a new password (min 10 characters, at least one number and one symbol)

If you do not receive the email within 10 minutes, check spam/junk and confirm you are using the email tied to the workspace. SSO (Business) users reset passwords through their identity provider, not Cyberfield.
...
3. Ensure `cyberfield.init` runs before `track` calls
...
Try an incognito window and disable ad blockers. If invoices fail to download, email billing@cyberfield-analytics.example with your workspace ID.

## Data deletion / GDPR request

Workspace admins can delete the workspace under Settings → Privacy → Delete workspace (irreversible after 7-day grace). For a subject access or erasure request for a specific user ID, email privacy@cyberfield-analytics.example.
